...
import {OrbitControls} from '../scripts/three-r127/examples/jsm/controls/OrbitControls.js';
import {GLTFLoader} from '../scripts/three-r127/examples/jsm/loaders/GLTFLoader.js';
import {EffectComposer} from '../scripts/three-r127/examples/jsm/postprocessing/EffectComposer.js';
import {RenderPass} from '../scripts/three-r127/examples/jsm/postprocessing/RenderPass.js';
...
export function preLoadModel(filename, name, kidSkip = true)
{
    ...
    me.three.loaders.gltf.load( '3d/models/' + filename + '.gltf', (gltf) =>
    {
        ...
        data.name = name;
        if( kidSkip )
        { data.model = gltf.scene.children[0]; } //kidskip skips the first child for models using inner groups
        else
...
{
    if( me.player.actions.objects.length < 1 )
    { return; } //No actions, return
...
function doFirstParentAction(object, distance)
{
    if( object === undefined )
    { return; }
    if( object.melon === undefined )
    { return; }
    let success = object.melon.doAction(distance);
    if( !success )
    { doFirstParentAction( object.parent, distance ); }
}
...
export function printToConsole(text)
{
    ...
    for( let i = 0; i < me.console.rows; i++ )
    {
        ...
        if( consoleFlop )
        {
            consoleText += '<br/><div class="console-flop">' + entry + '</div>';
            consoleFlop = false;
        }
        else
        {
...
export class MelonSuperObject extends THREE.Object3D
{
    ...
    setScale(scale)
    {
        if( this.melon === undefined )
        { MelonSuperObject.regenMelonObjectExtenders( this, false ); }
